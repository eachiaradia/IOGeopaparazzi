...
class TilingThread(QThread):
    ...
    def __init__(self, layers, extent, minZoom, maxZoom, width, height, transp, quality, format, outputPath, rootDir, antialiasing, tmsConvention, mbtilesCompression, jsonFile, overview, renderOutsideTiles, mapUrl, viewer,maxnumtiles):
        QThread.__init__(self, QThread.currentThread())
        self.warring_threshold_tiles_count = maxnumtiles
        self.mutex = QMutex()
    ...
    def run(self):
        ...
        if self.mode == 'DIR':
            ...
            if os.path.exists(self.output.filePath()):
              self.updateText.emit('%s already exists. It will be replaced.' %(self.output.fileName()))
              os.remove(self.output.filePath())
            else:
              self.updateText.emit('creating %s' %(self.output.fileName()))
        ...
        self.updateProgress.emit(int(0))
            
        useTMS = 1
        if self.tmsConvention:
            useTMS = -1
            
        self.updateText.emit('Searching tiles...')
        ...
        self.updateText.emit('Number of tiles: %d' %(len(self.tiles)))
        ...
        if len(self.tiles) > self.warring_threshold_tiles_count:
            ...
            self.updateText.emit('Number of tiles is over the limit %d' %(self.warring_threshold_tiles_count))
            return
        ...
        for t in self.tiles:
            tn+=1
            self.render(t)
            self.updateProgress.emit(int(100*float(tn)/len(self.tiles)))
            self.mutex.lock()
            s = self.stopMe
            self.mutex.unlock()
            if s == 1:
