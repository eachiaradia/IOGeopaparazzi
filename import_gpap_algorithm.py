...
class ImportGpapAlgorithm(QgsProcessingAlgorithm):
	...
	def processAlgorithm(self, parameters, context, feedback):
		...
		return {self.IMAGESOUT:imagesVL, self.BOOKMARKSOUT:bookMarksVL, self.NOTESOUT: formNotesVid, self.GPSLOGSOUT:gpsVL}
	...
	def ExportBookmarks(self,DBcursor):
		...
		(sink, dest_id) = self.parameterAsSink(self.PARAM, self.BOOKMARKSOUT, self.CONTEXT, fList,  QgsWkbTypes.Polygon , self.CRS)
		...
		for r in range(0,bookmarksTable.getNumOfRec()):
			...
			fet = QgsFeature()
			fet.setGeometry(QgsGeometry.fromWkt('POLYGON((%s %s, %s %s, %s %s, %s %s, %s %s))'%(w,n,e,n,e,s,w,s,w,n)))
	...
	def ExportGPSlogs(self,DBcursor, feedback = None):
		...
		(sink, dest_id) = self.parameterAsSink(self.PARAM, self.GPSLOGSOUT, self.CONTEXT, fList,  QgsWkbTypes.LineStringZ , self.CRS)
